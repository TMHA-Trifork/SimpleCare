...
export type BedWardPatientListItem =
  components["schemas"]["BedWardPatientListItem"];
...
export async function fetchBedWardPatients(): Promise<
...
export async function fetchBedWardPatient(
  patientId: string
): Promise<BedWardPatient> {
  const response = await fetch(
    getApiUrl(`/api/bed-ward/patients/${patientId}`)
  );
  if (!response.ok) {
    throw new Error("Network response was not ok");
  }
...
export async function dischargePatient(patientId: string): Promise<void> {
  const response = await fetch(
    getApiUrl(`/api/bed-ward/patients/${patientId}/discharge`),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
